...
public class capture_pic extends UiAutomatorTestCase
{
	public static final int DEFAULT_LOOP = 2000; /* Define a default loop count for testing. */
	TestSupport ts;
	...
	{  
		getAutomationSupport().sendStatus(-1, buildSummary.buildSummary(getParams(), 
				TestSummary.capture_pic, DEFAULT_LOOP));

		ts.launchapp("Camera");
        for(int i = 0; i < ts.getLoop(getParams(),DEFAULT_LOOP); i++)
        {
        	ts.mSleep(1);
        	UiObject capShutter = new UiObject(new UiSelector().descriptionContains("Shutter"));  
        	capShutter.click();
        }
